...
# MassiveDataSource is only available when `massive` is installed
try:
    from app.market.massive_client import MassiveDataSource
    _MASSIVE_AVAILABLE = True
except ImportError:
    _MASSIVE_AVAILABLE = False
    MassiveDataSource = None


@pytest.mark.skipif(not _MASSIVE_AVAILABLE, reason="massive package not installed")
class TestMassiveFactory:
    """Tests for MassiveDataSource creation in the factory."""
...
class TestSimulatorFactory:
    """Tests for SimulatorDataSource creation in the factory."""
